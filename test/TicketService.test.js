...
test('processes adult ticket purchases', () => {
  ...
  const purchaseSummary = service.purchaseTickets(1, new TicketTypeRequest('ADULT', 2));
  ...
  assert.deepEqual(purchaseSummary, {
    totalAmountPaid: 50,
    totalSeatsReserved: 2,
  });
});
...
test('calculates totals for mixed ticket purchases', () => {
  ...
  const purchaseSummary = service.purchaseTickets(
    1,
